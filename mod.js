  ...
  client.on("messageCreate", async (message) => {
    ...
    if (linkRegex.test(message.content)) {
      ...
      try {
        ...
        const aviso = await message.channel.send({
          content: `❌ ${message.author}, não envie links de convite aqui.`,
        });
    ...
    if (confirmacoes.has(message.author.id)) {
      ...
      if (comando.startsWith("!")) {
        return message.reply("⚠️ Responda 'sim' ou 'não' primeiro.");
      }
      ...
      if (resposta === "sim") {
        ...
        try {
          if (data.tipo === "ban") {
            ...
            await data.user.ban();
            return message.channel.send(`✅ ${data.user.user.tag} foi banido.`);
          }

          if (data.tipo === "kick") {
            if (!data.user.kickable) throw new Error();
            await data.user.kick();
            return message.channel.send(`✅ ${data.user.user.tag} foi expulso.`);
          }

          if (data.tipo === "castigar") {
            if (!data.user.moderatable) throw new Error();
            await data.user.timeout(10 * 60 * 1000);
            return message.channel.send(`✅ ${data.user.user.tag} ficou de castigo.`);
          }

        } catch (err) {
          console.log(err);
          return message.channel.send("❌ Não consegui executar essa ação.");
        }
      }

      if (["não", "nao"].includes(resposta)) {
        confirmacoes.delete(message.author.id);
        return message.channel.send("❌ Ação cancelada.");
      }
    ...
    // ================= VALIDAR ALVO =================
    function validarAlvo(message, user) {

      if (!user) return "❌ Marque alguém.";

      if (user.id === message.author.id)
        return "❌ Você não pode se punir.";

      if (user.id === message.guild.ownerId)
        return "❌ Não pode punir o dono.";
    ...
    if (comando === "!ban") {
      ...
      const erro = validarAlvo(message, user);
