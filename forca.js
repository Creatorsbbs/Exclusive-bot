...
const addXP = (id, v) => xp.set(id, (xp.get(id) || 0) + v);
const getXP = (id) => xp.get(id) || 0;
...
module.exports = (client) => {
    ...
    if (msg.content === "!forca-mp") {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          ...
          `Cada uma muda completamente o jogo!\n\n` +
          `Boa sorte 🍀`
        )
        .setColor("Purple");
  ...
  client.on("interactionCreate", async (i) => {
    ...
    if (i.customId.startsWith("entrar_")) {
      ...
      if (!j)
        return i.reply({ content: "❌ Sala não existe", ephemeral: true });

      if (j.players.length >= MAX_PLAYERS)
        return i.reply({ content: "❌ Cheia", ephemeral: true });
    ...
    if (i.customId.startsWith("iniciar_")) {
      ...
      if (i.user.id !== j.dono)
        return i.reply({ content: "❌ Só o dono", ephemeral: true });

      if (j.players.length < 2)
        return i.reply({ content: "❌ Precisa 2 jogadores", ephemeral: true });
    ...
    if (i.customId.startsWith("dica_")) {
      ...
      const id = i.customId.replace("dica_", "");
      const j = salas.get(id);

      if (!j) return;

      return i.reply({
        content: `💡 Dica: começa com **${j.palavra[0]}**`,
        ephemeral: true
      });
    }
    ...
    if (i.customId.startsWith("skip_")) {
      ...
      const id = i.customId.replace("skip_", "");
      const j = salas.get(id);
