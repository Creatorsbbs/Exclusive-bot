...
module.exports = (client) => {
  ...
  client.on("messageCreate", async (message) => {
    ...
    try {
      ...
      if (message.content === "!forca-mp") {

        // UMA SALA POR CANAL

        if (canaisAtivos.has(message.channel.id)) {

          return message.reply(
            "❌ Já existe uma partida ativa neste canal."
          );
        }
        ...
        salas.get(salaId).messageId = msg.id;
      ...
      if (interaction.isButton()) {
        ...
        if (interaction.customId.startsWith("iniciar_")) {
          ...
          if (interaction.user.id !== sala.dono) {
            ...
            return interaction.reply({
              content: "❌ Apenas o dono pode iniciar.",
              ephemeral: true
            });
          }
